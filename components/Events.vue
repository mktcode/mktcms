  ...
  <div class="bg-gray-100 py-24">
    <div class="container mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-10">
        <div v-for="post in posts" :key="post.id" class="bg-white rounded-2xl overflow-hidden">
          <img :src="`/files/${post.image}`" alt="event" class="w-full saturate-[75%] h-56 object-cover object-center">
          <div class="p-4 flex flex-col">
            <h2 class="text-xl font-bold text-gray-800 line-clamp-1">{{ post.title }}</h2>
            <p class="text-gray-400" v-if="post.date">
            ...
            <a v-if="post.url" :href="post.url" class="button ml-auto mt-4">zur Anmeldung</a>
          </div>
        </div>
      </div>
    </div>
